...
ZELONG_KEY  = os.environ.get("ZELONG_KEY", "")
    ...
    with closing(adb()) as c:
        ...
        for scope, username, voice_key, display_name, provider_voice in public:
            ...
            c.execute("""UPDATE audio_voices
                SET display_name=?, provider_voice=?, updated_at=?
                WHERE scope=? AND username=? AND voice_key=?""",
                (display_name, provider_voice, now, scope, username, voice_key))
...
def list_user_audio_voice_slots(username):
    with closing(adb()) as c:
        rows = c.execute("""SELECT s.id, s.username, s.user_id, s.slot_id, s.status, s.voice_id, COALESCE(s.reclone_count, 0) AS reclone_count, s.clone_started_at, s.previous_preview_url, s.clone_upload_at, s.clone_error,
...
def generate_doubao_preview(speaker_id, text=None, purpose="preview"):
    ...
    fn = "%s_%d.mp3" % (prefix, int(time.time() * 1000))
...
def clear_voice_preview(username, slot_id):
    ...
    with closing(adb()) as c:
        ...
        for r in rows:
            if r["preview_url"]:
                old_url = r["preview_url"]
                break
        c.execute("""UPDATE audio_voice_slots SET previous_preview_url=COALESCE(?, previous_preview_url)
            WHERE username=? AND slot_id=?""", (old_url, username, slot_id))
...
def same_preview_asset(a, b):
    if not a or not b:
        return False
    try:
        return urllib.parse.urlparse(a).path == urllib.parse.urlparse(b).path
    except Exception:
        return a == b
...
def check_clone_status(username, slot_id):
    ...
    slot_id = (slot_id or "").strip()
    with closing(adb()) as c:
        slot = c.execute("""SELECT id, slot_id, status, voice_id, clone_started_at, previous_preview_url, clone_upload_at, clone_error FROM audio_voice_slots
    ...
    if st == 3:
        with closing(adb()) as c:
            c.execute("UPDATE audio_voice_slots SET status='failed', clone_error=?, updated_at=? WHERE username=? AND slot_id=?",
                      ("\u8c46\u5305\u590d\u523b\u5931\u8d25", int(time.time()), username, slot_id))
            c.commit()
    ...
    try:
        ...
        if slot_id:
            try:
                with closing(adb()) as c:
                    c.execute("UPDATE audio_voice_slots SET status='failed', clone_error=?, updated_at=? WHERE username=? AND slot_id=?",
                              (str(e)[:400], int(time.time()), username, slot_id))
                    c.commit()
            except Exception:
                pass
    ...
    if voice_key in public_keys:
        with closing(adb()) as c:
            ...
            cur = c.execute("""INSERT INTO audio_voices
                (scope, username, voice_key, display_name, provider_voice, created_at, updated_at)
                VALUES('public','',?,?,?,?,?)""",
                (voice_key, display, voice_key, now, now))
            c.commit()
...
def record_audio_asset(job_id, username, result):
    if not result or result.get("type") != "audio":
        return
    now = int(time.time())
    voice_key = (result.get("voice") or DEFAULT_PUBLIC_VOICE).strip()
...
def _post_image_provider(path, data, ctype, base, key, proxy=True):
    req = urllib.request.Request(base + path, data=data,
                                 headers={"Authorization": "Bearer " + key, "Content-Type": ctype}, method="POST")
    if proxy:
        last = None
        for attempt in range(2):
            try:
                ...
                time.sleep(1.5)
        raise RuntimeError("上游生成服务连接中断，请稍后重试") from last
    with _NOPROXY.open(req, timeout=300) as r:
        return json.loads(r.read())
...
def gen_image(payload):
    ...
    def call_provider(provider):
        ...
        if img:
            ...
            if mask:
                files.append(("mask", "mask.png", base64.b64decode(mask)))
            body, ct = _multipart({"model": "gpt-image-2", "prompt": prompt, "size": size, "quality": "high", "n": "1"}, files)
            return _post_image_provider("/v1/images/edits", body, ct, base, key, proxy), ("inpaint" if mask else "img2img"), provider
        body = json.dumps({"model": "gpt-image-2", "prompt": prompt, "size": size, "quality": "high", "n": count}).encode()
        return _post_image_provider("/v1/images/generations", body, "application/json", base, key, proxy), "text2img", provider

    fallback_from = None
    try:
        d, mode, provider_used = call_provider(requested_provider)
    except Exception:
        if requested_provider != "openai":
            raise
        fallback_from = "openai"
        d, mode, provider_used = call_provider("zelong")
    ...
    for i, item in enumerate(d.get("data") or []):
        ...
        if item.get("b64_json"):
            ...
            opener = urllib.request.urlopen if provider_used == "openai" else _NOPROXY.open
            with opener(item["url"], timeout=120) as rr:
                raw = rr.read()
        else:
            raise RuntimeError("上游未返回图片数据")
        raw, dim = _normalize_image_bytes(raw, size)
        (OUT_DIR / fn).write_bytes(raw)
        files.append({"file": fn, "url": "/api/gen/file/" + fn})
        if dim:
            dimensions.append(dim)
    if not files:
        raise RuntimeError("上游未返回图片数据")
    result = {"type": "image", "mode": mode, "file": files[0]["file"], "url": files[0]["url"],
              "files": files, "ratio": ratio, "size": size, "prompt": prompt, "provider": provider_used}
    if dimensions:
...
def public_audio_voice_defs():
    return [
        ("public", "", key, PUBLIC_VOICE_LABELS.get(key, key), key)
        for key in PUBLIC_VOICE_KEYS
    ]
...
def gen_audio(payload):
    ...
    voice_key = (payload.get("voice") or DEFAULT_PUBLIC_VOICE).strip()
