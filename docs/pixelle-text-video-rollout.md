...
- 文案成片正式提交前必须调用 `/api/gen/text-video/quote`，向用户展示自动分镜后的 `scene_count`、费用明细和总点数，并取得明确确认。报价绑定账号、完整冻结参数、分镜数、费用和 10 分钟有效期；缺少报价、报价过期或参数/价格变化时不得扣点，必须重新报价确认。
- 规划阶段以每个口播分镜约 6 秒为目标，用所选语速估算并显示到 0.1 秒。完整文案优先按句末标点拆分，短句合并，长句按逗号或字符安全拆分，通常保持在 3–9 秒；不得截断、改写或遗漏原文，最终提交必须精确使用用户确认的分镜文本和选择。若 1000 字输入按目标时长会超过上游 20 分镜上限，主站按用户段落字数分配 20 个分镜额度并仅在各段落内部平衡拆分，绝不跨越用户空行；此兼容回退允许单镜头超过 9 秒，页面必须显示实际估算时长。
