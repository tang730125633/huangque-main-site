...
6. 字段目标覆盖《黄雀IP人设定位信息采集表》的基本/职业背景、核心经历、内容方向、
...
7. 模块 1-4 确认完成后自动生成受账号权限保护的《IP人设定位背景档案》PDF，包含原始背景、
   ...
   已完成画像。成果 API 只有在 PDF `ready` 且画像 revision 一致时才返回查看地址；失败或
   待生成时只返回重试入口，前端不创建 iframe，也不把 PDF 描述符重复显示为普通成果。
   合法最大长度回答和候选方案使用可跨页段落，不截断用户信息。
8. DeepSeek 必须把低信息、玩笑、页面操作和元描述判定为 `clarify`；后端再执行字段级
   质量门禁。“参考”“回顾模块”“用户选择”“一句话的雏形”等内容不能写入画像。
9. 模块完成前必须满足关键事实组：定位包含职业、案例、受众、痛点和差异证据；人设包含
   三个性格词和表达风格；价值包含实际原话、自我介绍、信任依据、产品及阶段目标；故事
   至少有一条包含处境、行动和结果的真实经历。明确跳过非关键问题仍然允许。
10. 已完成的旧画像会按同一规则只读审计。存在问题时隐藏内容创作入口和旧 PDF，展示
    “完善画像”；用户确认后才从第一个问题开始补采，并跳过已有的合格答案。旧字段
    `identity` 兼容映射为 `basic_context`，PDF schema 变化会触发重新生成。
