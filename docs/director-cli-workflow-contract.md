...
- HQ CLI 0.14.0 已实现编导工作流创建/读取/分镜 CAS 更新/导出、单镜头图片/视频/口播、冻结方案生产与同款复刻。
- 短剧顾问、角色定妆、预飞、自动草稿、交付和完结五阶段动作已进入同一服务端目录；无水印下载由 `dl` 专用安全传输执行。
- Precision 数字人完整运行仍为 `planned`，在服务端持久子步骤、退款和恢复闭环完成前不得由 CLI 宣称可调用。
...
3. 单镜头图片/视频/口播、一次冻结方案生成一个成品，以及同款复刻。
...
每一阶段必须从当时最新 `main` 开始，锁定 Head SHA，限制 diff，并在合并后才开始依赖它的下一阶段。真实扣点验收仍必须单独取得用户确认。
