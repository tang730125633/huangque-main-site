...
新增的两个跨服务私有 SQLite 数据库。当前状态：**等待固定业务/Schema SHA 的 LU-003
阶段一批准**。本文不是合并、部署、迁移、重启、真实 Provider 调用或数据库整体恢复授权。

## 固定对象与审批

- 目标分支：`main`
- 审核基线：合并前重新固定最新 `main`
- 业务/Schema SHA：等待本记录与全部业务修复一次性提交后填写
- 维护窗口：等待 LU-003 指定未来的 `Asia/Shanghai` 起止时间
- Owner：`LU-003`
- 执行负责人：`LU-003`
- 回滚负责人：`LU-003`
- 阶段一批准：等待固定 SHA 后在 PR 评论中批准数据库路径、窗口、停写、WAL 一致性备份、
  空库/升级/幂等验证和失败回滚；批准链接随后只追加到本文。
