...
本机隔离回归共 195 项通过：基础及任务相关 172 项、真实 PostgreSQL 配置/任务入口 10 项、admin config store 13 项，无跳过。

新增入口测试分别使用 SQLite 和 PostgreSQL 配置存储，真实执行 HTTP handler、任务数据库写入、入队及独立 worker。认证、扣点系统和供应商网络为隔离替身，断言 URL 与凭据摘要，不输出密钥、不真实扣点、不调用付费生成。
...
最后一组必须提供隔离 HQ_DATABASE_URL；本机可先通过 tests.pg_harness.postgres_url() 启动实例。所有测试必须在隔离环境执行。上线仍按原 PR 的迁移、代码发布和单线路灰度步骤独立审批执行。
