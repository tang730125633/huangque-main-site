...
const batch = {
  ...
  topic: '企业做 Agent 前先梳理流程', goal: '建立信任', status: 'quoted', created_at: 1787720000,
  plans: [
...
(async () => {
  ...
  if (Object.values(report).some((item) => item.width > item.viewport || item.messages < 2 || item.plans < 2 || !item.confirm || item.total !== '10 点' || item.tabs !== 3 || item.aiEntry !== 'creator-agent.html' || item.aiLabel !== 'AI 创作助手')) process.exitCode = 1;
