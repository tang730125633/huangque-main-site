...
class CreatorAgentContractTests(unittest.TestCase):
    ...
    def test_service_is_loopback_and_release_is_atomic(self):
        ...
        self.assertLess(release_mode, current_switch)
        ...
        self.assertIn("画像需要完善", self.page)
        self.assertIn("画像待完善", self.page)
        self.assertIn("repair_profile", self.service)
        self.assertIn("profile_quality_issues", self.service)
        store = (ROOT / "server/creator_agent/store.py").read_text(encoding="utf-8")
