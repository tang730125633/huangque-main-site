...
class ThreeBirdPrototypeTest(unittest.TestCase):
    def test_three_bird_contract(self):
        ...
        self.assertIn("data-bird-video", html)
        self.assertIn("liquid-bird-h3-alpha.webm", html)
        self.assertIn("liquid-bird-h3.mp4", html)
        self.assertTrue((PROTOTYPE / "public" / "assets" / "liquid-bird-h3-alpha.webm").is_file())
        self.assertTrue((PROTOTYPE / "public" / "assets" / "liquid-bird-h3.mp4").is_file())
        self.assertIn('type="module"', html)
        self.assertIn("UnrealBloomPass", script)
        self.assertNotIn("GLTFLoader", script)
        self.assertNotIn("huangque-eagle.glb", script)
        self.assertNotIn("AnimationMixer", script)
