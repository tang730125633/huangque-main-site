...
function createRuntime(plan, storage){
  ...
  const fetch=(url,options={})=>{
    ...
    if(url==='/api/gen/audio/voices'){const index=requests.voices.length;requests.voices.push({username:authUsername});return plan.voices?plan.voices(index,authUsername):Promise.resolve(response(200,{items:[{scope:'public',voice_key:'S_d21F8OR62',display_name:'温柔女声',preview_url:'/public.mp3',provider_voice:'longwan'},{scope:'personal',voice_key:'vip_alice',display_name:'我的复刻音色',preview_url:'/personal.mp3',provider_voice:'cosyvoice-v3.5-plus-alice'}]}))}
