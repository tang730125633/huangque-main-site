...
function createRuntime(plan, storage){
  ...
  const fetch=(url,options={})=>{
    ...
    if(url==='/api/gen/matrix-template/templates')return Promise.resolve(response(200,{templates:[{id:'full-overlay-bold',name:'沉浸强标题',tags:['原生'],font_selectable:true},{id:'poster-split',name:'三段式活动海报',tags:['原生'],font_selectable:true},{id:'native-bold',name:'默认原生大字',tags:['默认'],font_selectable:true},{id:'minimal-headline',name:'极简标题',tags:['极简'],font_selectable:true},{id:'ref-01-fixture-01',name:'参考模板',description:'绿色粗描边手写标题',tags:['内置字体'],engine:'hyperframes',font_mode:'template_locked',font_selectable:false,variant:'v01'},{id:'nine-grid-reveal',name:'九宫格开场·全屏展示',description:'九格依次显现，随后切换三段全屏素材',tags:['九宫格'],engine:'hyperframes',font_mode:'template_locked',font_selectable:false,variant:'nine-grid',duration_mode:'fixed_12',required_visuals:9,bgm_mode:'bound',bgm_optional:true},{id:'triple-strip-shutter',name:'三横屏开场·光栅快切',description:'三条横屏同时开场，随后五段全屏光栅快切',tags:['固定节奏'],engine:'hyperframes',font_mode:'template_locked',font_selectable:false,variant:'triple-strip',duration_mode:'fixed',fixed_duration_seconds:17.6,required_visuals:8,bgm_mode:'bound',bgm_optional:true},{id:'yellow-banner-zoom',name:'黄条标题·变幅冲击',description:'黄条信息标题与三段素材变幅冲击',tags:['固定节奏'],engine:'hyperframes',font_mode:'template_locked',font_selectable:false,variant:'yellow-banner',duration_mode:'fixed',fixed_duration_seconds:10.066666666666666,required_visuals:3,bgm_mode:'bound',bgm_optional:true}],fonts:[{value:'',label:'自动搭配',source:'automatic'},{value:'Noto Sans SC',label:'思源黑体',source:'bundled'},{value:'AaHouDiHei',label:'Aa厚底黑',source:'private'}],default_template:'native-bold',default_font:'',max_batch_size:5,engine_concurrency:{ffmpeg:5,hyperframes:2},cost:5}));
...
async function scenarioVoiceoverSubmission(){
  response.extraTemplates=[{id:'bilingual-stagger-salon',name:'Bilingual',variant:'bilingual-stagger',engine:'hyperframes'}];
...
async function scenarioVoiceoverBgmSubmission(){
  ...
  await flush(30);runtime.get('topText').value='配音标题';runtime.get('bottomText').value='评论区扣关键词';runtime.get('topText').listeners.input[0]();runtime.get('bottomText').listeners.input[0]();runtime.get('voiceoverText').value='这是一段带背景音乐的口播文案';runtime.get('voiceoverText').listeners.input[0]();runtime.get('voiceoverBgmEnabled').checked=true;runtime.get('voiceoverBgmEnabled').listeners.change[0]();runtime.get('voiceoverBgmVolume').value='35';runtime.get('voiceoverBgmVolume').listeners.input[0]();runtime.get('generateBtn').onclick();await flush(30);
...
async function scenarioVoiceoverValidation(){
  response.extraTemplates=[{id:'bilingual-stagger-salon',name:'Bilingual',variant:'bilingual-stagger',engine:'hyperframes'}];
  const runtime=createRuntime({post:()=>Promise.reject(new Error('unused')),poll:()=>Promise.reject(new Error('unused'))},new Map());
  await flush(30);runtime.get('voiceTemplateTab').onclick();runtime.get('topText').value='配音标题';runtime.get('bottomText').value='评论区扣关键词';runtime.get('topText').listeners.input[0]();runtime.get('bottomText').listeners.input[0]();const before={auth:runtime.requests.auth.length,post:runtime.requests.post.length,confirm:runtime.requests.confirm.length};runtime.get('generateBtn').onclick();await flush(20);
...
async function scenarioUncertainRecoversAutomatically(){
  ...
  const storage=new Map([['hq-matrix-template-pending-v2:alice',JSON.stringify({owner:'alice',started_at:Date.now(),items:[{key,body:{top_text:'待确认标题',bottom_text:'待确认行动文案',template_id:'native-bold',bgm:true},job_id:'',status:'uncertain',result:null,error:'提交响应丢失',refund_status:''}]})]]);
...
async function scenarioErrorResponseWithJobIdPollsTerminal(){
  const storage=new Map();
  const runtime=createRuntime({
    post:()=>Promise.resolve(response(503,{code:'reconcile_pending',job_id:8086,detail:'结果待确认'})),
    poll:()=>Promise.resolve(response(200,{status:'error',error:'预检失败',refunded:true,refund_status:'已退款'})),
  },storage);
  await fillAndSubmit(runtime);await flush(30);
  const card=runtime.get('batchResults').children[0];
  return {posts:runtime.requests.post.length,polls:runtime.requests.poll.length,error:card&&card.children[1].textContent,refund:card&&card.children[2].textContent,cleared:pendingCleared(storage)};
}

async function scenarioStaleUnavailableSubmissionPauses(){
  const key='matrix-template-stale-unavailable';
  const storage=new Map([['hq-matrix-template-pending-v2:alice',JSON.stringify({owner:'alice',started_at:Date.now()-1107000,items:[{key,body:{top_text:'旧提交标题',bottom_text:'旧提交行动文案',template_id:'native-bold',bgm:true},job_id:'',status:'submitting',result:null,error:'',refund_status:''}]})]]);
  const runtime=createRuntime({post:()=>Promise.resolve(response(503,{code:'reconcile_pending',detail:'服务暂不可用'})),poll:()=>Promise.reject(new Error('no job must not poll'))},storage);
  await flush(30);
  const pending=JSON.parse(storage.get('hq-matrix-template-pending-v2:alice'));
  return {posts:runtime.requests.post.length,polls:runtime.requests.poll.length,status:runtime.get('status').textContent,itemStatus:pending.items[0].status,pending:!pendingCleared(storage),activeTimers:runtime.timers.filter(timer=>timer.active).length,action:actionState(runtime)};
}

async function scenarioPausedSubmissionResumesOnClick(){
  const key='matrix-template-paused-key';
  const storage=new Map([['hq-matrix-template-pending-v2:alice',JSON.stringify({owner:'alice',started_at:Date.now()-1107000,items:[{key,body:{top_text:'暂停标题',bottom_text:'暂停行动文案',template_id:'native-bold',bgm:true},job_id:'',status:'needs_attention',reconcile_started_at:Date.now()-1107000,reconcile_attempts:8,result:null,error:'自动确认已暂停',refund_status:''}]})]]);
  const runtime=createRuntime({post:()=>Promise.resolve(response(200,{job_id:8087})),poll:()=>Promise.resolve(response(200,{status:'done',result:{video_url:'/paused-recovered-video',duration:8}}))},storage);
  await flush(30);const before={posts:runtime.requests.post.length,status:runtime.get('status').textContent,action:actionState(runtime)};
  runtime.get('generateBtn').onclick();await flush(30);
  return {before,posts:runtime.requests.post.length,keys:runtime.requests.post.map(call=>call.options.headers['Idempotency-Key']),src:runtime.get('video').src,cleared:pendingCleared(storage)};
}

async function scenarioPausedSubmissionEndsUncharged(){
  const key='matrix-template-paused-uncharged';
  const storage=new Map([['hq-matrix-template-pending-v2:alice',JSON.stringify({owner:'alice',started_at:Date.now()-1107000,items:[{key,body:{top_text:'未受理标题',bottom_text:'未受理行动文案',template_id:'native-bold',bgm:true},job_id:'',status:'needs_attention',reconcile_started_at:Date.now()-1107000,reconcile_attempts:8,result:null,error:'自动确认已暂停',refund_status:''}]})]]);
  const runtime=createRuntime({post:()=>Promise.resolve(response(404,{code:'idempotency_not_found',detail:'原提交未进入扣点阶段，请重新生成',operation_terminal:true,accepted:false,charged:false})),poll:()=>Promise.reject(new Error('uncharged claim must not poll'))},storage);
  await flush(30);const before={posts:runtime.requests.post.length,status:runtime.get('status').textContent,action:actionState(runtime)};
  runtime.get('generateBtn').onclick();await flush(30);
  const card=runtime.get('batchResults').children[0];
  return {before,posts:runtime.requests.post.length,polls:runtime.requests.poll.length,error:card&&card.children[1].textContent,refund:card&&card.children[2].textContent,status:runtime.get('status').textContent,cleared:pendingCleared(storage)};
}
...
async function scenarioShellRecovery(){
  const missing=createRuntime({post:()=>Promise.reject(new Error('unused')),poll:()=>Promise.reject(new Error('unused'))},new Map());
  await missing.triggerWindow('load');await missing.runTimer();await flush();
  await missing.triggerWindow('load');await missing.runTimer();await flush();
  const retries=missing.head.children.filter(node=>node.tagName==='SCRIPT').map(node=>({src:node.src,marker:node.getAttribute('data-hq-shell-recovery')}));
  const healthy=createRuntime({post:()=>Promise.reject(new Error('unused')),poll:()=>Promise.reject(new Error('unused'))},new Map());
  healthy.get('hqSideNav');await healthy.triggerWindow('load');await healthy.runTimer();await flush();
...
async function scenarioMotionV3(){
  ...
  const runtime=createRuntime({post:()=>Promise.resolve(response(200,{job_id:42})),poll:()=>Promise.resolve(response(200,{status:'done',result:{video_url:'/test.mp4'}}))},new Map());
  await flush(40);
  ...
  const selected={top:runtime.get('topTextLabel').textContent,bottom:runtime.get('bottomTextLabel').textContent,voice:!runtime.get('voiceoverPanel').hidden,voiceLocked:!runtime.has('voiceoverEnabled'),bgmLocked:runtime.get('voiceoverBgmEnabled').disabled};
  runtime.get('generateBtn').onclick();await flush(30);
  const body=runtime.requests.post.length?JSON.parse(runtime.requests.post[0].options.body):null;
  for(let i=0;i<10&&!pendingCleared(runtime.storage);i++){await runtime.runTimer();await flush(20)}
  runtime.get('bgmTemplateTab').onclick();
  runtime.get('generateBtn').onclick();await flush(30);
  const bgmBody=runtime.requests.post.length>1?JSON.parse(runtime.requests.post[1].options.body):null;
  return {selected,body,bgmBody,restoredLabel:runtime.get('bottomTextLabel').textContent,bgmVoiceOff:runtime.get('voiceoverPanel').hidden,toggleAbsent:!runtime.has('voiceoverEnabled')};
}
