...
class ContentDomainTests(unittest.TestCase):
    ...
    def test_core_does_not_own_domain_handlers(self):
        ...
        # 软上限防 core 膨胀（真守卫是上面的 hasattr 域处理器检查）；1250→1300→1330→1400→1420：
        # 视频任务池按 mode 三分(口播/motion/慢池)属 core 队列基础设施、非域逻辑，合理留 core；
        # reclaim_orphaned_running(启动回收重启遗留孤儿→退点)属 core 任务生命周期、紧挨 reaper；
        # user_notifications 建表(init_db)+读路由(do_GET 分发)属 core 基础设施，业务逻辑在 notifications.py。
        self.assertLess(len(core_path.read_text(encoding="utf-8").splitlines()), 1420)
