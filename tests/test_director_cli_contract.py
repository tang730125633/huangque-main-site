...
class DirectorCLIContractTests(unittest.TestCase):
    ...
    def test_current_digital_human_v2_endpoints_are_registered(self):
        html = (ROOT / "site" / "workbench" / "digital-human-oneclick.html").read_text(
            encoding="utf-8"
        )
        current = {
            value.split("?", 1)[0]
            for value in re.findall(r"/api/gen/digital-human-v2/[A-Za-z0-9_/?=&.-]+", html)
        }
        covered = {
            endpoint.split(":", 1)[1].split("?", 1)[0]
            for action in contract.DIGITAL_HUMAN_ONECLICK_ACTIONS
            for endpoint in action["server_endpoints"]
            if ":/api/gen/digital-human-v2/" in endpoint
        }
        self.assertTrue(current)
        self.assertEqual(set(), current - covered)
