...
class PointTransferUiTests(unittest.TestCase):
    def test_settings_exposes_transfer_form_and_history(self):
        ...
        self.assertIn('id="pointsGiftCard" class="settings-card" data-points-ui hidden', SETTINGS)
        self.assertIn("user.membership_tier==='partner'||user.membership_tier==='initiator'", SETTINGS)
        self.assertIn(
            "if(res.data.user.points_billing_enabled&&canGiftPoints(res.data.user)) loadGiftHistory(true);",
            SETTINGS,
        )
