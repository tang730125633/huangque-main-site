...
class ScriptSubmissionGuardTests(unittest.TestCase):
    ...
    def test_breakdown_partial_refund_intent_precedes_done_and_is_recoverable(self):
        ...
        self.assertIn("def _optional_points_call(name, *args):", source)
        self.assertIn("callback = getattr(_domains()[1], name, None)", source)
        self.assertIn('_optional_points_call(\n                "prepare_breakdown_batch_refund"', source)
