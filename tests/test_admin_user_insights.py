...
class AdminUserInsightsFrontendTests(unittest.TestCase):
    ...
    def test_support_layout_uses_sidebar_dashboard_and_customer_drawer(self):
        ...
        for marker in (
            ...
            "if(state.module==='dashboard')loadDashboard(true);else if(state.module==='servers')loadServers(true);else if(state.module==='operations')load(true)", "credential_version",
            "var routeUnverified=", "&force=1",
            "evidence:registryRouteEvidence({key:meta.key}", "verification_state:verification",
