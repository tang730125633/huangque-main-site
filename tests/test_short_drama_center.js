...
test('客户端使用 Cookie 会话并支持安全删除独立短剧', async () => {
  ...
  assert.equal(calls[2].url, '/api/gen/short-drama/project/delete');
  assert.equal(calls[2].options.method, 'POST');
  assert.deepEqual(JSON.parse(calls[2].options.body), {project_id:'project-1', revision:4});
