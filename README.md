...
| AI 配音 | `audio.html` | 阿里百炼 CosyVoice（公共音色 + **VIP 声音克隆**）、OpenAI TTS 兼容兜底 |
...
1. **接口调试**：
   ...
   - 外部 API 密钥：**12 个渠道**全可测——带密钥的真调一次上游验证密钥有效性；COS 等签名类只测连通与延迟（标注"仅连通"）；**拨测顺带显示余额**（HeyGen 额度 / RunningHub 币 / TikHub 美元）
...
| 阿里百炼 CosyVoice | TTS、声音克隆 | 直连（DashScope API Key） |
