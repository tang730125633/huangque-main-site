    ...
    document.querySelector('[data-module="managedChannels"]').addEventListener('submit',async e=>{
      ...
      try{
        ...
        if(f.id==='cmForm'){
          ...
          delete v.materials;const affected=routeMappings().filter(m=>mappingChannels(m).includes(editing.id)).map(m=>m.label||m.operation_id||m.front);if(!confirm('保存渠道 '+v.name+' 为新版本？\n实际模型：'+(editing.model||'未配置')+' → '+v.model+'\n接单：'+(v.enabled?'启用':'停用')+'\n关联功能：'+(affected.join('、')||'尚未配置映射')+'\n已有任务保留原版本，仅新任务受影响。'))return;const replacement=editing._replacement,saved=await post('save',v);if(replacement){for(const operationId of replacement.operations||[]){const current=routeMappings().find(m=>m.operation_id===operationId)||{},existing=mappingChannels(current).filter(id=>id!==saved.id),managed=current.state==='managed';await post('operation-mapping',{operation_id:operationId,state:managed?'managed':'shadow',channels:managed?[...existing,saved.id]:[saved.id,...existing],expected_revision:Number(current.revision||0)})}await post('test',{id:saved.id,kind:'connection'});await post('test',{id:saved.id,kind:'auth'})}workspace.close();
        ...
        toast(editing?._replacement?'新 Key 已保存并加入影子候选；连接与鉴权检测已开始':'已保存');await load();
