...
(function(){
  ...
  function build(){
    ...
    if(notifyBtn){
      ...
      notifyWrap.style.cssText='position:relative; display:flex; align-items:center;';
      notifyBtn.parentNode.insertBefore(notifyWrap, notifyBtn);
      notifyWrap.appendChild(notifyBtn);
      var notifyDot=document.createElement('span');
      notifyDot.style.cssText='position:absolute; top:6px; right:6px; width:8px; height:8px; border-radius:50%; background:#e7b24c; box-shadow:0 0 0 2px rgba(7,11,19,.92); display:none;';
      notifyWrap.appendChild(notifyDot);
      var notifyPop=document.createElement('div');
      notifyPop.style.cssText='position:absolute; top:46px; right:0; width:min(340px, calc(100vw - 28px)); max-height:360px; overflow:auto; display:none; border:1px solid rgba(148,164,187,.14); border-radius:14px; background:linear-gradient(180deg, rgba(16,24,39,.98), rgba(9,13,22,.98)); box-shadow:0 24px 60px rgba(0,0,0,.45); z-index:120;';
      notifyWrap.appendChild(notifyPop);
      var notifyOpen=false;
      function bellStoreRead(){
        try{ var raw=localStorage.getItem('hq_jobs'); var arr=JSON.parse(raw||'[]'); return Array.isArray(arr)?arr:[]; }catch(e){ return []; }
      }
      function bellJobs(limit){
        try{
          if(window.HQTasks && window.HQTasks.listRecent){ return window.HQTasks.listRecent({kind:'leads', limit:limit||5})||[]; }
        }catch(e){}
        return bellStoreRead().filter(function(job){ return !job || !job.kind || job.kind==='leads'; }).sort(function(a,b){ return (b.updatedAt||b.createdAt||0)-(a.updatedAt||a.createdAt||0); }).slice(0, limit||5);
      }
      function bellSummary(job){
        if(!job) return '暂无状态';
        if(job.status==='done') return '已完成 · '+escapeHtml(job.leads_count||0)+' 个客户';
        if(job.status==='error' || job.status==='failed') return '失败 · '+escapeHtml(job.error||'任务失败');
        if(job.status==='queued') return '排队中';
        if(job.status==='pending' || job.status==='running') return '进行中';
        return escapeHtml(job.status||'待处理');
      }
      function bellStatusColor(job){
        if(!job) return '#94a4bb';
        if(job.status==='done') return '#2dd4bf';
        if(job.status==='error' || job.status==='failed') return '#f4708a';
        return '#e7b24c';
      }
      function markBellRead(id){
        try{ if(window.HQTasks && window.HQTasks.markRead){ window.HQTasks.markRead(id); return; } }catch(e){}
        try{
          var arr=bellStoreRead();
          var changed=false;
          arr.forEach(function(job){ if(String(job.id)===String(id)){ job.unread=false; changed=true; } });
          if(changed) localStorage.setItem('hq_jobs', JSON.stringify(arr));
        }catch(e){}
      }
      function renderBell(){
        var items=bellJobs(5);
        var hot=items.some(function(job){ return !!job.unread || /^(queued|pending|running)$/.test(job.status||''); });
        notifyDot.style.display=hot?'block':'none';
        if(!items.length){
          notifyPop.innerHTML='<div style="padding:14px 16px; border-bottom:1px solid rgba(148,164,187,.08);"><div style="font-size:13px; font-weight:700; color:#eaf1fa;">获客通知</div><div style="margin-top:4px; font-size:12px; color:#94a4bb;">最近任务会显示在这里</div></div><div style="padding:22px 16px; text-align:center; color:#5c6b82; font-size:12.5px;">暂无获客任务</div>';
          return;
        }
        notifyPop.innerHTML='<div style="padding:14px 16px; border-bottom:1px solid rgba(148,164,187,.08); position:sticky; top:0; background:linear-gradient(180deg, rgba(16,24,39,.98), rgba(9,13,22,.98));"><div style="font-size:13px; font-weight:700; color:#eaf1fa;">获客通知</div><div style="margin-top:4px; font-size:12px; color:#94a4bb;">点击可回到对应任务</div></div>'+
          items.map(function(job){
            var title=escapeHtml(job.title||job.keyword||'获客任务');
            var href=safeUrl((job&&job.href)||('leads.html#task='+encodeURIComponent(String(job.id||''))));
            var statusColor=bellStatusColor(job);
            var unread=job&&job.unread;
            return '<a href="'+escapeAttr(href)+'" data-task-id="'+escapeAttr(job.id||'')+'" style="display:block; padding:12px 16px; text-decoration:none; border-bottom:1px solid rgba(148,164,187,.08); background:'+(unread?'rgba(231,178,76,.05)':'transparent')+';">'+
              '<div style="display:flex; align-items:flex-start; gap:10px;">'+
                '<span style="margin-top:3px; width:8px; height:8px; border-radius:50%; flex:none; background:'+statusColor+';"></span>'+
                '<div style="min-width:0; flex:1;">'+
                  '<div style="display:flex; align-items:center; gap:8px;">'+
                    '<span style="font-size:13px; font-weight:600; color:#eaf1fa; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">'+title+'</span>'+
                    (unread?'<span style="flex:none; width:6px; height:6px; border-radius:50%; background:#e7b24c;"></span>':'')+
                  '</div>'+
                  '<div style="margin-top:4px; font-size:12px; color:'+statusColor+';">'+bellSummary(job)+'</div>'+
                '</div>'+
                '<span style="display:flex; width:12px; color:#5c6b82; flex:none;">'+icon('chevron','12px')+'</span>'+
              '</div>'+
            '</a>';
          }).join('');
      ...
      notifyBtn.onclick=function(ev){
        ev.preventDefault(); ev.stopPropagation();
        notifyOpen=!notifyOpen;
        if(notifyOpen){
          renderBell();
          notifyPop.style.display='block';
          notifyBtn.style.background='rgba(148,164,187,.08)';
        }else closeBell();
      };
      document.addEventListener('click', function(ev){ if(!notifyWrap.contains(ev.target)) closeBell(); });
      document.addEventListener('keydown', function(ev){ if(ev.key==='Escape') closeBell(); });
      try{ window.addEventListener('storage', function(e){ if(!e || e.key==='hq_jobs') renderBell(); }); }catch(e){}
      try{ if(window.HQTasks && window.HQTasks.onChange) window.HQTasks.onChange(renderBell); }catch(e){}
      renderBell();
    }
