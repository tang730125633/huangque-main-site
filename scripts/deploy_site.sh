...
echo "▸ 3/3 部署内容后端 content_api.py + tikhub.py + func_names.py + heygen_oauth.py + 重启 huangque-content / huangque-admin"
rsync -az --rsync-path="sudo rsync" \
  -e "ssh -i $KEY -o IdentitiesOnly=yes -o BatchMode=yes" \
  "$ROOT/server/content_api.py" "$ROOT/server/tikhub.py" "$ROOT/server/func_names.py" "$ROOT/server/heygen_oauth.py" \
