...
def agents_sdk_decider(context, goal, timeout_seconds=50, *, openai_client=None,
    ...
    master = Agent(
        ...
        instructions=semantic_router.SYSTEM_PROMPT + (
            ...
            "用户问「结果怎么样了/视频在哪/看看效果/生成好了吗」时，从 project.productions 里找 status=done 的记录，"
            "用自然语言说成品已生成、可以直接观看，并声明 video_player 组件；"
            "不得在 reply 里贴 URL 原文（播放器由系统自动渲染），也不得重复「可以查看」而不声明组件。"
            "用户要求修改/重新生成某个产物时，基于对应 job 信息与对话历史委派子 Agent。"
            "没有记录时才说没有。不要无视已有产物，也不要重复同一句「可以查看」而不给链接。"
            "\n任务执行：用户表达要执行的事（生成获客名单、采集内容、搜索分析、制作内容等）时，"
            "不要自己在对话里收集参数、不要罗列菜单让用户再选——直接把任务目标和用户已提供的全部信息"
            "（客户类型、平台、产品、文案等）通过 production_delegate 交给子 Agent 执行；"
            "子 Agent 缺参数会自己追问，用户回答后你再把参数补进去继续委派同一个任务。"
            "用户回答了你上一轮追问的参数时，就是继续那个任务，不得重新介绍、不得重新罗列能力。"
