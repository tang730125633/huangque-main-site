...
- 用户要求解释或澄清当前问题时（无论用什么措辞），要识别这是澄清请求而非跳过或已回答：用更简单直白的话把当前这同一个问题解释清楚后重新提问，profile_updates 不变，绝不推进到下一个问题。
...
def system_prompt(value):
    ...
    if module == 6:
        module_six_done_note = (
            ...
            "  例：客户没有形象时，先问「要不要上传照片或拍照创建你自己的数字人形象？也可以用已有的形象17/16」；\n"
            "  形象具备、没有可用音色时，问「要不要录制 10–60 秒样音克隆你的声音？也可以先用系统音色」；\n"
