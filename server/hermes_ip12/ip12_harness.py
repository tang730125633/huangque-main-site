...
- 用户说“没听懂”“说清楚一点”“什么意思”“再解释下”等要求澄清时，绝不是跳过，也绝不能换下一个问题：用更简单直白的话把【当前这同一个问题】解释清楚后重新提问，profile_updates 不变。
...
- 用户没有明确指定其他时长时，默认单篇 60–90 秒、250–350 字；只有用户明确选择其他时长才覆盖默认值。"""
    ...
    if module in MODULE_FIELD_OWNERSHIP:
        ...
        if missing:
            ...
            module_field_rules = """
- 本模块名下还有未采集字段：%s。进入本模块第一个断点之前，必须先自然采集这些缺失字段：decision=ask_follow_up、checkpoint=0，每轮只问一个，已采集或用户明确跳过的不得以任何说法重复追问；全部补齐后才能 propose_checkpoint。
- 用户说“没听懂”“说清楚一点”“什么意思”等要求澄清时，绝不是跳过，也绝不能换下一个字段：用更简单直白的话把当前这同一个字段解释清楚后重新提问，profile_updates 不变。""" % labels
    return f"""你是黄雀 IP12 的中立 IP 咨询教练，适用于任何职业和行业。
