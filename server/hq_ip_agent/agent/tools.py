...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "m5_topics",
            "description": (
                "模块5·选题生成。报告定稿后系统已自动确认（confirmed=true），直接调用即可；"
                "工具内部会校验确认状态，未确认会明确报错。输入直接从已确认 PDF 提取"
...
def dispatch(name: str, args: dict, session_id: str) -> dict:
    ...
    if name == "get_report":
        ...
        if opts:
            ...
            if meta.get("status") == "final":
                confirm = "已自动确认" if meta.get("confirmed") else "尚未确认"
                note = ("当前报告状态：已定稿（final），确认状态=" + confirm +
                        "。最终推荐=用户所选方案"
                        f"{meta.get('chosen') or ''}《{meta.get('chosen_title') or ''}》——"
                        "只提这一个方案，不要再把三套方案摆出来让用户重新选，"
                        "也不要重提草稿阶段的推荐款。"
                        "（已自动确认时直接推进模块5，**绝不让用户去页面点确认**；"
                        "尚未确认时如实说明原因，不要说成已完成。）")
            else:
    ...
    if name == "get_m5m6":
        ...
        note = "模块5/6 当前状态与内容全文"
        for key, label in (("m5", "选题"), ("m6", "文案")):
            st = (meta.get(key) or {}).get("status")
            if st and st != "ready" and not out.get(key + "_topics") and not out.get(key + "_scripts"):
                note += (f"；模块{key[1]} {label}还没到 ready（当前 {st}）："
                         "绝不要声称「清单/文案已排好、在状态区」，如实说还在生成/审核中")
        return _ok(out, note=note)
