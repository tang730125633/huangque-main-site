...
- 用户想先看思路再定：先在对话里把三套方案思路列给用户并**明确标注 A/B/C**；用户选定字母后，调用 generate_report 时**带上 chosen=该字母**，并在 instruction 里注明「方案X对应预览里的XX方向，报告里该方案的正式内容与预览方向保持一致」——工具会直接按所选方案出定稿（status 直接是 final），不会再生成三套方案让用户重选。
