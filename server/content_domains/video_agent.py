...
MODULE_CATALOG = (
    "可交接的六个制作模块：\n"
    "1. talking / 数字人口播：已就绪数字人形象，或当前账号已验证的私有人物图片 + 非空文案 + 音色，生成自然口型视频；现成音频不能替代文案和音色。\n"
    "2. motion / 动作模仿：数字人形象 + 参考视频，模仿动作、节奏和运镜。\n"
    "3. story / 剧情故事：至少一个已就绪电影化身 + 剧情描述，制作角色一致的电影感短片；普通参考图片或视频只能作为可选参考，不能替代电影化身。\n"
...
def _system_prompt():
    return (
        ...
        "已有已确认人物图片但尚无 avatar_id 时，不得说用户没有提供人物图片。talking 口播可直接使用"
        "当前账号已验证的私有 img_ upload_id，但还必须有非空文案和音色；现成音频不能替代文案或音色。"
        "motion 动作模仿和 story 剧情故事必须使用当前账号已就绪的 avatar_id，普通参考图片或视频不能"
        "替代电影化身；只有这些模块需要先创建并等待形象就绪。你不得自动提交任何"
