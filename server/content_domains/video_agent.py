...
MODULE_CATALOG = (
    "可交接的六个制作模块：\n"
    "1. talking / 数字人口播：当前账号已核验私有人物图片（image_upload_id）或已就绪"
    "数字人形象（avatar_id）二选一，并同时提供非空文案 text 与音色 voice。\n"
    "2. motion / 动作模仿：当前账号已就绪数字人形象 + 已核验参考视频，模仿动作、节奏和运镜。\n"
    "3. story / 剧情故事：当前账号一个或多个已就绪数字人形象 + 剧情描述；普通参考图仅可选，"
    "不能替代数字人形象。\n"
...
def _system_prompt():
    return (
        ...
        "talking 可直接使用当前账号已核验的私有人物图片 image_upload_id，无需先创建 avatar；也可"
        "使用已就绪的 avatar_id，两者必须二选一。talking 必须同时具备非空文案 text 和音色 voice，"
        "现成音频不能替代这两个必填字段。story 必须使用当前账号已就绪的数字人形象 avatar_id 或 "
        "avatar_ids，普通参考图片只能作为可选参考，不能替代形象；motion 同样必须使用已就绪的 "
        "avatar_id，并提供已核验参考视频。已有已确认人物图片但尚无 avatar_id 时，只有在 story 或"
        "motion 场景才应提示先创建数字人形象；不得说用户没有提供人物图片。你不得自动提交任何"
