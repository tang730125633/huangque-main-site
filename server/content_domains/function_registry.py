...
VIDEO_FUNCTIONS = [
    ...
    },
    {
        ...
        "dependencies": [
            {"key": "xai", "role": "生成线路", "requirement": "required",
    ...
    "modes": [
        {
            ...
            "key": "canvas.image.seedream.std", "name": "黄雀引擎 1",
            "entrypoints": [_endpoint("POST", "/api/gen/image"), _endpoint("GET", "/api/gen/job/{id}")],
            "flag_keys": ["image"],
            "dependencies": [{"key": "seedance", "role": "Seedream 主生成", "requirement": "required", "credential_source": "env"}],
            "task_match": {"kind": "image", "source_page": "canvas", "provider": "seedream"},
            "evidence_contract": {"not_applicable": ["provider_task"]},
            "price_keys": ["image.seedream.std.hd"],
            "smoke_inputs": ["固定产品提示词", "画布图片节点结果写回"],
            "validation": _image_validation(),
        },
    ],
    ...
    "modes": [
        {
            ...
            "flag_keys": ["grok_video"],
            "dependencies": [{"key": "xai", "role": "主生成", "requirement": "required", "credential_source": "pool"}],
            ...
            "key": "canvas.video.minimax", "name": "麦克视频",
            "entrypoints": [_endpoint("POST", "/api/gen/xiaole_video"), _endpoint("GET", "/api/gen/job/{id}")],
            "flag_keys": ["minimax_h3_video"],
            "dependencies": [{"key": "minimax", "role": "主生成", "requirement": "required", "credential_source": "pool"}],
            "task_match": {"kind": "xiaole_video", "source_page": "canvas", "channel": "minimax"},
            "price_keys": ["video.minimax_h3.768p"],
            "smoke_inputs": ["固定产品提示词", "画布视频节点业务结果"],
            "validation": _validation({
                "mode": "minimax", "prompt": QA_PROMPT, "duration": 5,
