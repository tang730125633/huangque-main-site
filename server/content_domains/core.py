...
ZELONG2_BASE = os.environ.get("ZELONG2_BASE", "https://api.zelong.vip")
ZELONG2_IMAGE_MODEL = os.environ.get("ZELONG2_IMAGE_MODEL", "").strip()
_NOPROXY = urllib.request.build_opener(urllib.request.ProxyHandler({}))     # 直连(绕过 HTTPS_PROXY)，给国内中转用
COPY_BASE = os.environ.get("COPY_BASE") or OPENAI_BASE
COPY_API_KEY = os.environ.get("COPY_API_KEY") or OPENAI_KEY
COPY_API_STYLE = (os.environ.get("COPY_API_STYLE") or "chat_completions").strip().lower()
    ...
    def do_POST(self):
        ...
        if kind is not None:
            ...
            try:
                ...
                if not is_still_route: idem_key = _idempotency_key(self.headers.get("Idempotency-Key")) if kind in {"image", "banana", "audio", "video", "tryon", "xiaole_video", "sora_video", "cinematic", "script_to_video", "breakdown", "canvas_agent"} else ""
