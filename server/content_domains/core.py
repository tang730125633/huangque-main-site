...
VERIFY_CACHE_TTL = max(0.0, _env_float("VERIFY_CACHE_TTL", 8))
VERIFY_CACHE_MAX = max(1, _env_int("VERIFY_CACHE_MAX", 2048))
...
def _verify_cache_get(token, now):
    if not VERIFY_CACHE_TTL:
        return None
...
def _verify_cache_set(token, user, now):
    if not VERIFY_CACHE_TTL or not isinstance(user, dict):
        return
    with _verify_cache_lock:
        if len(_verify_cache) >= VERIFY_CACHE_MAX:
            for k, (exp, _) in list(_verify_cache.items()):
                if exp <= now:
...
def verify(token):
    ...
    now = time.time()
    cached = _verify_cache_get(token, now)
    if cached:
        return cached
    try:
        ...
        with urllib.request.urlopen(req, timeout=6) as r:
            ...
            if user:
                _verify_cache_set(token, user, now)
                return dict(user)
    except Exception:
        _verify_cache_drop(token)
        return None
    _verify_cache_drop(token)
    return None
