...
print("渠道: %s  id=%s v%s" % (it["name"], it["id"][:12], it["version"]))
for k in ("connection","auth","full"):
    s,b=call("POST","/api/admin/channel-manager/test",{"id":it["id"],"kind":k},t=120)
    print("  %-11s → HTTP %s %s" % (k, s, b[:90]))
    time.sleep(4)
print("\n=== 90 秒后查 ===")
time.sleep(90)
d=json.loads(call("GET","/api/admin/channel-manager",t=200)[1])
...
for r_ in (it.get("checks") or []):
    print("  %-11s %-9s %s" % (r_.get("kind"), r_.get("state"), str(r_.get("detail"))[:110]))
